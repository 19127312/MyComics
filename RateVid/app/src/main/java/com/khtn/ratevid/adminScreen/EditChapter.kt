...
import com.khtn.ratevid.adapter.ChosenEditImageAdapter
...
class EditChapter : AppCompatActivity() {
    private val INCREASE_CHAPTER=200

    lateinit var imgsList : ArrayList<ModelEditImage>
    lateinit var adapter: ChosenEditImageAdapter
    var comicID=""
    var chapterNumber=0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        imgsList= ArrayList<ModelEditImage>()
        adapter = ChosenEditImageAdapter(this,imgsList)
        ...
        ref.addListenerForSingleValueEvent(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (item in snapshot.children){
                    val modelComic = item.getValue(ModelEditImage::class.java)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode==1111 &&resultCode== Activity.RESULT_OK&& data!=null){
            ...
            imgsList.add(ModelEditImage(imgsList.size+1,filepath,"Waiting to upload"))
